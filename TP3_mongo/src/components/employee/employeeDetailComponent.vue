<template>
    <div class="w-full px-4 border-b mt-2">
        ...
        <div class="w-full px-4  mt-2 flex mb-12 ">
            <div class="flex flex-col items-center justify-center ">
                <div class="grid grid-cols-2 gap-2 w-96 ">
                    <div class="badge bg-gray-500 gap-2 hover:cursor-pointer justify-self-center w-full"
                        v-for="employee in useEmployee.paginatedEmployees" :key="employee.employeeID"
                        @click="useEmployee.getEmployeeDetail(employee)">
                        <svg v-if="employee.check" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        <p class="text-white"> {{ employee.firstName }} - {{ employee.lastName }}
                        </p>
                    </div>
                </div>
            ...
            <div v-if="useEmployee.employeeDetail">
                <p class="font-bold mb-2 text-center">
                    {{ useEmployee.employeeDetail.firstName }} {{ useEmployee.employeeDetail.lastName }}
                </p>
                    ...
                    <tbody>
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{
                                useEmployee.employeeDetail.title
                                }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{
                                useEmployee.employeeDetail.titleOfCourtesy }}</td>
                            <td class="border border-gray-300 px-4 py-2">
                                {{ new Date(useEmployee.employeeDetail.birthDate).toLocaleDateString('fr-FR') }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">
                                {{ new Date(useEmployee.employeeDetail.hireDate).toLocaleDateString('fr-FR') }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">{{ useEmployee.employeeDetail.address }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">{{ useEmployee.employeeDetail.city }}</td>
                            <td class="border border-gray-300 px-4 py-2">
                                {{ useEmployee.employeeDetail.regionDetails[0].regionDescription === "NULL" ? "" :
                                    useEmployee.employeeDetail.regionDetails[0].regionDescription
                                }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">{{ useEmployee.employeeDetail.postalCode }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">{{ useEmployee.employeeDetail.country }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2">{{ useEmployee.employeeDetail.homePhone }}
                            </td>
                        </tr>
                    </tbody>
                </table>
...
onBeforeUnmount(() => {
    useEmployee.employeeDetail = null
});
</script>